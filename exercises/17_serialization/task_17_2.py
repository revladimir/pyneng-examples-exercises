...
def parse_config(sh_ver_output):
    regex = (
        r"Cisco IOS .*? Version (?P<ios>\S+), .*"
        r"uptime is (?P<uptime>[\w ,]+)\n.*"
    ...
    match = re.search(regex, sh_ver_output, re.DOTALL)
    if match:
        return match.group("ios", "uptime", "image")
...
def write_inventory_to_csv(data_filenames, csv_filename):
    ...
    with open(csv_filename, "w") as f:
        ...
        for filename in data_filenames:
            hostname = re.search("sh_version_(?P<hostname>\S+).txt", filename).group(1)
            with open(filename) as f:
                parsed_data = parse_config(f.read())
                if parsed_data:
                    writer.writerow([hostname] + list(parsed_data))


if __name__ == "__main__":
    sh_ver_file_names = glob.glob("sh_vers*")
    write_inventory_to_csv(sh_ver_file_names, "routers_inventory.csv")
